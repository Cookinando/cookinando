    ...
    return (
        <nav className={`navbar flex justify-between lg:flex-row lg:px-8 items-start ${menuOpen ? 'bg-transparent fixed inset-0' : ''}`} aria-label="Global">
            ...
            >
                {!isAuthenticated && (
                    <>
                        <li className= "pointer-events-auto w-screen flex justify-center lg:hidden"> <Link to="#" className="cursor-pointer hover:text-gray-300" onClick={() => setMenuOpen(false)}> <img className= "h-5" src={closeIcon} alt="Close icon" /></Link></li>
